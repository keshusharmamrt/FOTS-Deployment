...
def orignal_model_predict(detector,recognizer,size,image):
      img=image.copy()
...
def float16_model_predict(det,rec,size,image):
        start_time=time.time()
        img=image.copy()
        img1=img.astype('float32')
        det[0].set_tensor(det[1][0]['index'],np.expand_dims(img1,axis=0))
...
def main():
  ...
  detector_orignal,recognizer_orignal,size_orignal=load_main_models()
  det_dynamic,rec_dynamic,size_dynamic=load_dynamic_quantized_models()
